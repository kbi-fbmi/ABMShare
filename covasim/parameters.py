...
immunity_custom_pars={
    "variants":{
        "get_variant_choices":None,
        "variant_pars":None,
    },
    "immunity":{
        "get_vaccine_choices":None,
        "cross_immunity":None,
        "vaccine_variant_pars":None,
        "vaccine_dose_pars":None,
    },
}
...
def make_pars(set_prognoses=False, prog_by_age=True, version=None, **kwargs):
    ...
    if version is not None:
        ...
        for key in pars: # Only loop over keys that have been populated
...
def get_variant_choices():
    ...
    #Added above original covasim for handling custom variant
    if immunity_custom_pars["variants"]["get_variant_choices"]:
        choices.update(immunity_custom_pars["variants"]["get_variant_choices"])
...
def get_vaccine_choices():
    ...
    #Added above original covasim for handling custom vaccine
    if immunity_custom_pars["immunity"]["get_vaccine_choices"]:
        choices.update(immunity_custom_pars["immunity"]["get_vaccine_choices"])
    mapping = {name:key for key,synonyms in choices.items() for name in synonyms} # Flip from key:value to value:key
    return choices, mapping
...
def get_variant_pars(default=False, variant=None, variants:list=None):
    ...
    # Append variants to pars #DONE
    new_variants=immunity_custom_pars["variants"]["variant_pars"]
    if new_variants:
        for var in new_variants:
            pars.update({var:new_variants[var]})
...
def get_cross_immunity(default=False, variant=None):
    ...
    list of variants to append to the default list of variants
    """
    pars = dict(
    ...
    new_variants=immunity_custom_pars["immunity"]["cross_immunity"]
    # Append to older variants first, then append new ones #DONE
    if new_variants:
        for var in new_variants:
            # Check if there is defined 1 value for the same variant
            if var not in new_variants[var] or new_variants[var][var] not in [1,1.0,"1","1.0"]:
                new_variants[var].update({var:1.0})
            # Add new value
            if var not in pars:
                # Update old values with new ones
                for orig_var in pars:
                    if var not in pars[orig_var]:
                        pars[orig_var].update({var:new_variants[var][orig_var]})
                # # Add new value
                pars.update({var:new_variants[var]})
...
def get_vaccine_variant_pars(default=False, vaccine=None, vaccines:list=None):
    ...
    # Append vaccines to pars
    new_variants=immunity_custom_pars["immunity"]["vaccine_variant_pars"]
    original_pars=pars.copy()
    if new_variants:
        for vaccine in new_variants:
            if vaccine in pars:
                for var in original_pars:
                    if var in new_variants:
                        pars[vaccine].update({vac:exut.safe_eval_fraction(val) for vac,val in new_variants[vaccine].items() if not pd.isna(val)})
                        break
            else:
                for key, val in new_variants[vaccine].items():
                    if not pd.isna(val):
                        new_variants[vaccine][key]=exut.safe_eval_fraction(val)
                pars.update({vaccine:new_variants[vaccine]})
...
def get_vaccine_dose_pars(default=False, vaccine=None):
    ...
    # Append vaccines to pars
    parsed=immunity_custom_pars["immunity"]["vaccine_dose_pars"]
    nab_init_keys=["dist","par1","par2"]
    if parsed:
        for vaccine in parsed:
            nab_init = {}
            for key, val in (parsed[vaccine].copy()).items():
                if key in nab_init_keys:
                    nab_init[key]=val
                    del parsed[vaccine][key]
            pars.update({vaccine:parsed[vaccine]})
            pars[vaccine].update({"nab_init":nab_init})
