...
def check_immunity(people, variants=None):
    ...
    for variant in variants:
        ...
        # ADDED for fixing waning while mobility_sim runned step by step()
        if np.any(np.isnan(people.recovered_variant[was_inf])):
            positions=np.where(np.logical_not(np.isnan(people.exposed_variant[was_inf])))[0]
            if np.size(positions)==0:
                positions=people.exposed_by_variant[variant][was_inf]
            if len(positions)>0:
                recovered_variant[positions]=people.exposed_variant[was_inf][positions]
        if np.any(np.isnan(recovered_variant)):
            try:
                positions=np.where(np.isnan(recovered_variant))[variant]
            except:
                positions=np.where(np.isnan(recovered_variant))
