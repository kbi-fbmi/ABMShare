...
from . import data, defaults, logger
...
def load_location(specific_location:str=None, state_location:str=None, country_location:str=None, revert_to_default=None,different_location=None,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        different_location (str)   : Location for region/ state configuration files
        region_config (str)      : Filepath to configuration file
    Returns:
        str: A filename for where the location data reside.
    ...
    if region_config:
        try:
            location_object=data.load_configuration_from_filepath(region_config)
            logger.debug(f"Loaded configuration from {region_config}")
            return location_object
        except:
            try:
                location_object=data.load_configuration_from_filepath(parent_config)
                return location_object
            except:
                logger.debug(f"Cannot load configuration from {region_config}")
                return None
    ...
    if defaults.settings.config_dirpath:
        different_location = str(defaults.settings.config_dirpath)
    try:
        location_object = data.load_location_from_filepath(location_filepath,different_location=different_location)
...
def read_age_bracket_distr(datadir=None, location=None, state_location=None, country_location=None, nbrackets=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        region_config (string)        : file path to user specified age bracket distribution data
    ...
    if region_config:
        try:
            location_data = load_location(region_config=region_config)
        except:
            try:
                location_data = load_location(region_config=region_config,parent_config=parent_config)
            except:
                print("An error occured due to loading age bracket distribution")
    else:
...
def get_smoothed_single_year_age_distr(datadir=None, location=None, state_location=None, country_location=None, nbrackets=None, file_path=None, use_default=False, window_length=7,region_config:str=None,parent_config:str=None):
    ...
    """
    if region_config:
        age_bracket_distr = read_age_bracket_distr(region_config=region_config)
        age_brackets = get_census_age_brackets(region_config=region_config)
...
def get_household_size_distr(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        region_config (string)        : file path to user specified household size distribution data
        parent_config (string)        : file path to user specified household size distribution data
    ...
    if region_config:
        try:
            ...
            location_data = load_location(region_config=parent_config)
    else:
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.household_size_distribution is None or len(location_data.household_size_distribution) == 0):
        location_data = load_location(region_config=parent_config)

    elif use_default and (location_data.household_size_distribution is None or len(location_data.household_size_distribution) == 0):
...
def get_head_age_brackets(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        use_default (bool)        : if True, try to first use the other parameters to find data specific to the location under study, otherwise returns default data drawing from the settings.location, settings.state_location, settings.country_location.
        region_config (string)        : file path to user specified head age brackets data
        parent_config (string)        : file path to user specified head age brackets data
    ...
    if region_config:
        ...
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.household_head_age_brackets is None or len(location_data.household_head_age_brackets) == 0):
        location_data = load_location(region_config=parent_config)

    # Use default if no data for this parameter.
    elif use_default and (location_data.household_head_age_brackets is None or len(location_data.household_head_age_brackets) == 0):
...
def get_head_age_by_size_distr(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        use_default (bool)        : if True, try to first use the other parameters to find data specific to the location under study, otherwise returns default data drawing from settings.location, settings.state_location, settings.country_location.
        region_config (string)        : file path to user specified age of the head of the household by household size distribution data
        parent_config (string)        : file path to user specified age of the head of the household by household size distribution data
    Returns:
    ...
    if region_config:
        ...
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.household_head_age_distribution_by_family_size is None or len(location_data.household_head_age_distribution_by_family_size) == 0):
        location_data = load_location(region_config=parent_config)

    # Use default if no data for this parameter.
    elif use_default and (location_data.household_head_age_distribution_by_family_size is None or len(location_data.household_head_age_distribution_by_family_size) == 0):
...
def get_census_age_brackets(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False, nbrackets=None,region_config:str=None,parent_config:str=None):
...
def get_school_enrollment_rates(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        use_default (bool)        : if True, try to first use the other parameters to find data specific to the location under study, otherwise returns default data drawing from settings.location, settings.state_location, settings.country_location.
        region_config (string)        : file path to user specified school enrollment by age data
        parent_config (string)        : file path to user specified school enrollment by age data
    ...
    if region_config:
        ...
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.enrollment_rates_by_age is None or len(location_data.enrollment_rates_by_age) == 0):
        location_data = load_location(region_config=parent_config)
    # Use default if no data for this parameter.
    elif use_default and (location_data.enrollment_rates_by_age is None or len(location_data.enrollment_rates_by_age) == 0):
...
def get_school_size_brackets(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        use_default (bool)        : if True, try to first use the other parameters to find data specific to the location under study, otherwise returns default data drawing from settings.location, settings.state_location, settings.country_location.
        region_config (string)        : file path to user specified school size brackets data
        parent_config (string)        : file path to user specified school size brackets data
    ...
    if region_config:
        ...
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.school_size_brackets is None or len(location_data.school_size_brackets) == 0):
        location_data = load_location(region_config=parent_config)
    # Use default if no data for this parameter.
    elif use_default and (location_data.school_size_brackets is None or len(location_data.school_size_brackets) == 0):
...
def get_school_size_distr_by_brackets(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        use_default (bool)        : if True, try to first use the other parameters to find data specific to the location under study, otherwise returns default data drawing from settings.location, settings.state_location, settings.country_location.
        region_config (string)        : file path to user specified school size distribution data
        parent_config (string)        : file path to user specified school size distribution data
    ...
    if region_config:
        ...
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.school_size_distribution is None or len(location_data.school_size_distribution) == 0):
        location_data = load_location(region_config=parent_config)
    # Use default if no data for this parameter.
    elif use_default and (location_data.school_size_distribution is None or len(location_data.school_size_distribution) == 0):
...
def get_school_type_age_ranges(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        region_config (string)        : file path to user specified distribution data
        parent_config (string)        : file path to user specified distribution data
    ...
    if region_config:
        ...
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.school_types_by_age is None or len(location_data.school_types_by_age) == 0):
        location_data = load_location(region_config=parent_config)
    # Use default if no data for this parameter.
    elif use_default and (location_data.school_types_by_age is None or len(location_data.school_types_by_age) == 0):
...
def get_school_size_distr_by_type(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    if region_config:
        ...
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.school_size_distribution_by_type is None or len(location_data.school_size_distribution_by_type) == 0):
        location_data = load_location(region_config=parent_config)
    # Use default if no data for this parameter.
    elif use_default and (location_data.school_size_distribution_by_type is None or len(location_data.school_size_distribution_by_type) == 0):
...
def get_employment_rates(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        use_default (bool)        : if True, try to first use the other parameters to find data specific to the location under study, otherwise returns default data drawing from settings.location, settings.state_location, settings.country_location.
        region_config (string)        : file path to user specified employment by age data
        parent_config (string)        : file path to user specified employment by age data
    ...
    if region_config:
        ...
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.employment_rates_by_age is None or len(location_data.employment_rates_by_age) == 0):
        location_data = load_location(region_config=parent_config)
    # Use default if no data for this parameter.
    elif use_default and (location_data.employment_rates_by_age is None or len(location_data.employment_rates_by_age) == 0):
...
def get_workplace_size_brackets(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        use_default (bool)        : if True, try to first use the other parameters to find data specific to the location under study, otherwise returns default data drawing from settings.location, settings.state_location, settings.country_location.
        region_config (string)        : file path to user specified workplace size brackets data
        parent_config (string)        : file path to user specified workplace size brackets data
    ...
    if region_config:
        ...
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.workplace_size_counts_by_num_personnel is None or len(location_data.workplace_size_counts_by_num_personnel) == 0):
        location_data = load_location(region_config=parent_config)
    # Use default if no data for this parameter.
    elif use_default and (location_data.workplace_size_counts_by_num_personnel is None or len(location_data.workplace_size_counts_by_num_personnel) == 0):
...
def get_workplace_size_distr_by_brackets(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        use_default (bool)        : if True, try to first use the other parameters to find data specific to the location under study, otherwise returns default data drawing from settings.location, settings.state_location, settings.country_location.
        region_config (string)        : file path to user specified workplace size distribution data
        parent_config (string)        : file path to user specified workplace size distribution data
    Returns:
...
def get_long_term_care_facility_residents_distr(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        use_default (bool)        : if True, try to first use the other parameters to find data specific to the location under study, otherwise returns default data drawing from settings.location, settings.state_location, settings.country_location.
        region_config (string)        : file path to user specified LTCF resident size distribution data
        parent_config (string)        : file path to user specified LTCF resident size distribution data
    ...
    if region_config:
        ...
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.ltcf_num_residents_distribution is None or len(location_data.ltcf_num_residents_distribution) == 0):
        location_data = load_location(region_config=parent_config)
    # Use default if no data for this parameter.
    elif use_default and (location_data.ltcf_num_residents_distribution is None or len(location_data.ltcf_num_residents_distribution) == 0):
...
def get_long_term_care_facility_residents_distr_brackets(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        use_default (bool)        : if True, try to first use the other parameters to find data specific to the location under study, otherwise returns default data drawing from settings.location, settings.state_location, settings.country_location.
        region_config (string)        : file path to user specified LTCF resident size brackets data
        parent_config (string)        : file path to user specified LTCF resident size brackets data
    Returns:
...
def get_long_term_care_facility_resident_to_staff_ratios_distr(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    Args:
        ...
        use_default (bool)        : if True, try to first use the other parameters to find data specific to the location under study, otherwise returns default data drawing from settings.location, settings.state_location, settings.country_location.
        region_config (string)        : file path to user specified resident to staff ratio distribution data
        parent_config (string)        : file path to user specified resident to staff ratio distribution data
    Returns:
        dict: A dictionary of the distribution of residents per facility for
        Long Term Care Facilities.
    ...
    if region_config:
        ...
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.ltcf_resident_to_staff_ratio_distribution is None or len(location_data.ltcf_resident_to_staff_ratio_distribution) == 0):
        location_data = load_location(region_config=parent_config)
    # Use default if no data for this parameter.
    elif use_default and (location_data.ltcf_resident_to_staff_ratio_distribution is None or len(location_data.ltcf_resident_to_staff_ratio_distribution) == 0):
...
def get_long_term_care_facility_resident_to_staff_ratios_brackets(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
...
def get_long_term_care_facility_use_rates(datadir=None, location=None, state_location=None, country_location=None, file_path=None, use_default=False,region_config:str=None,parent_config:str=None):
    ...
    if region_config:
        ...
        location_data = load_location(location, state_location, country_location, revert_to_default=use_default)
    # Use default if no data for this parameter.
    if parent_config and (location_data.ltcf_use_rate_distribution is None or len(location_data.ltcf_use_rate_distribution) == 0):
        location_data = load_location(region_config=parent_config)
    # Use default if no data for this parameter.
    elif use_default and (location_data.ltcf_use_rate_distribution is None or len(location_data.ltcf_use_rate_distribution) == 0):
