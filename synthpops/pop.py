...
class Pop(sc.prettyobj):
    ...
    def __init__(self,
        ...
        Args:
            ...
            config_dirpath (string)                 : if there is externaly folder for loaded configuration files
        ...
        self.location_code    = location_code #Only for holding this info
        ...
        # Handle data
        if self.country_location is None and self.region_config_path is None:
        ...
        # Handle config_dirpath, configpath and parent config if provided
        if self.datadir:
            defaults.settings.config_dirpath = self.config_dirpath
        if self.region_config_path:
            defaults.settings.region_config = self.region_config_path
            self.loc_pars.region_config = self.region_config_path
        if self.parent_config_path:
            defaults.settings.parent_config = self.parent_config_path
    ...
    def generate(self):
        ...
        # Added above
        region_config                   = self.region_config_path
        parent_config                   = self.parent_config_path
        ...
        uids_in_school, uids_in_school_by_age, ages_in_school_count = spsch.get_uids_in_school(datadir, n_nonltcf, location, state_location, country_location, age_by_uid, homes_by_uids, use_default=use_default,region_config=region_config,parent_config=parent_config)  # this will call in school enrollment rates
