...
class BaseSim(ParsObj):
    ...
    def export_results(self, for_json=True, filename=None, indent=2,key="all", *args, **kwargs):
        ...
        if for_json:
            resdict["timeseries_keys"] = self.result_keys(which=key)
    ...
    def to_df(self, date_index=False,key="all"):
        ...
        """
        resdict = self.export_results(for_json=False,key=key)
    ...
    def to_excel(self, filename=None, skip_pars=None, key="all"):
        """Export parameters and results as Excel format

        Args:
            filename  (str): if None, return string; else, write to file
            skip_pars (list): if provided, a custom list parameters to exclude
            keys      (str): if provided then export only main,variant or all parameters
        Returns:
        ...
        # Export results
        result_df = self.to_df(date_index=True,key=key)
    ...
    def _get_ia(self, which, label=None, partial=False, as_list=False, as_inds=False, die=True, first=False):
        ...
        for label in labels:
            if sc.isnumber(label):
                ...
                for ind,ia_obj in enumerate(ia_list):
                    if sc.isstring(label) and ia_obj.label == label or (partial and (label in str(ia_obj.label))) or isinstance(label, type) and isinstance(ia_obj, label):
